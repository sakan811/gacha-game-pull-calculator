...
class BannerConfig:
    ...
    def validate(self) -> None:
        ...
        if not self._MIN_BASE_RATE <= self.base_rate <= self._MAX_BASE_RATE:
            raise ValueError(f"Base rate must be between {self._MIN_BASE_RATE} and {self._MAX_BASE_RATE}")
            
        if not self._MIN_FOUR_STAR_RATE <= self.four_star_rate <= self._MAX_FOUR_STAR_RATE:
            raise ValueError(f"Four star rate must be between {self._MIN_FOUR_STAR_RATE} and {self._MAX_FOUR_STAR_RATE}")
            
        if not self._MIN_PITY <= self.soft_pity_start_after <= self.hard_pity <= self._MAX_PITY:
            raise ValueError(f"Soft pity must be between {self._MIN_PITY} and hard pity, hard pity must be between soft pity and {self._MAX_PITY}")
