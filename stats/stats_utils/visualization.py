...
class BannerVisualizer:
    ...
    def create_distribution_plot(self, data, game_type, banner_type, save_path=None):
        ...
        # Get soft pity start from banner config
        config_key = f"{game_type}_{banner_type}"
        soft_pity_start = BANNER_CONFIGS[config_key].soft_pity_start_after
        ...
        soft_pity_prob = (
            data.loc[
                data["Roll Number"] == soft_pity_start, "Probability per Roll"
            ].values[0]
            if soft_pity_start in data["Roll Number"].values
            else 0
        )
        if soft_pity_prob > 0:
            ax.annotate(
                f"Soft Pity: {soft_pity_prob:.1%}",
    ...
    def create_cumulative_plot(self, data, game_type, banner_type, save_path=None):
        ...
        # Add reference lines
        fifty_percent_pull = np.interp(
            0.5, data["Cumulative Probability"], data["Roll Number"]
        )

        ax.axhline(0.5, color=colors[4], linestyle=":", label="50% Chance")
        ax.axhline(1.0, color=colors[2], linestyle="-.", label="100% Guarantee")

        # Get hard pity from banner config
        config_key = f"{game_type}_{banner_type}"
        hard_pity = BANNER_CONFIGS[config_key].hard_pity

        ax.axvline(hard_pity, color=colors[1], linestyle="--", label="Hard Pity")

        ax.annotate(
            f"50% at pull {int(fifty_percent_pull)}",
