...
class StatsRunner:
    ...
    def process_all_banners(self):
        ...
        if not self.banner_configs:
            self.logger.warning("No banner configurations provided. Skipping processing.")
            return
        ...
        for config_key in self.banner_configs:
            try:
                ...
                self.logger.info(f"Processing banner config: {config_key} (Game: {game_input_type}, Banner: {banner_input_type})") 
                
                banner_analyzer = BannerStats(game_type=game_input_type, banner_type=banner_input_type)
                
                file_paths = banner_analyzer.save_statistics_csv()
                
                if file_paths:
                    self.logger.info(f"Successfully saved stats for {banner_analyzer.game_type} {banner_analyzer.banner_type}:")
                    for metric, path in file_paths.items():
                        self.logger.info(f"  {metric}: {path}")
                else:
                    self.logger.info(f"Stats processed for {banner_analyzer.game_type} {banner_analyzer.banner_type}, but no CSV files were reported as generated.")
