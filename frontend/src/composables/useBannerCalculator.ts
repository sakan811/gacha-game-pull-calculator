...
export function useBannerCalculator() {
  ...
  watch(gameType, () => {
    if (bannerType.value === "light_cone" && gameType.value !== "star_rail") {
      ...
      bannerType.value =
        gameType.value === "star_rail" ? "light_cone" : "w_engine";
    } else if (
      bannerType.value === "w_engine" &&
      gameType.value !== "zenless"
    ) {
      bannerType.value =
        gameType.value === "star_rail" ? "light_cone" : "weapon";
    }
  });
  ...
  const maxPityForBannerType = computed(() => {
    if (gameType.value === "star_rail") {
      ...
      return bannerType.value === "w_engine" || bannerType.value === "bangboo"
        ? 80
        : 90;
    } else {
  ...
  async function calculateProbability() {
    ...
    try {
      const response = await fetch(
        `/api/${gameType.value}/${bannerType.value}`,
        {
          method: "POST",
