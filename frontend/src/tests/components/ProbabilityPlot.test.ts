...
describe('ProbabilityPlot Component', () => {
  ...
  const renderComponent = (props = mockBannerProps.standard) => {
    return render(ProbabilityPlot, { props }) as unknown as { container: HTMLElement, rerender: (props: any) => void };
  };

  const triggerChartUpdate = async (container: HTMLElement): Promise<void> => {
    const element = container.querySelector('[data-testid="probability-plots"]');
    if (!element) {
    ...
    testCaseGroups.basicRender.forEach(({ name, assertion }) => {
      it(name, () => {
        const { container } = renderComponent({
          ...
          totalPulls: mockBannerProps.standard.currentPity + mockBannerProps.standard.plannedPulls
        });
        assertion(container);
      });
    });
