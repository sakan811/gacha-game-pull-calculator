<script lang="ts">
  ...
  function validatePity(value: number) {
    const maxPity = bannerType === 'light_cone' ? 79 : 89;
    if (value < 0) return 0;
    if (value > maxPity) return maxPity;
    return value;
  }
  ...
  async function calculateProbability() {
    try {
      ...
      const response = await fetch(`/api/${bannerType}`, {
        method: 'POST',
...
<main class="main-container">
  <div class="layout-container {!showPlots ? 'calculator-centered' : 'calculator-with-plots'}">
        ...
        <form on:submit|preventDefault={calculateProbability} class="form-group">
          ...
          <div class="form-input-container">
            ...
            <input
              ...
              min="0"
              max={bannerType === 'light_cone' ? 79 : 89}
              class="form-input"
            />
            <p class="form-helper">
              Number of pulls since last 5★ (0-{bannerType === 'light_cone' ? '79' : '89'})
            </p>
          </div>
    ...
    <div class="plots-wrapper {showPlots ? 'plots-wrapper-visible' : 'plots-wrapper-hidden'}">
      {#if result}
        <ProbabilityPlot
          {bannerType}
          {currentPity}
