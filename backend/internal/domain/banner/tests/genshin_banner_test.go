...
func TestGenshinStandardBanner(t *testing.T) {
	tests := []testCase{
		{
			name: "base rates are correct",
			testFunc: func(t *testing.T) {
				config := banner.GetConfig(banner.GenshinStandard)
		...
		},
		{
			...
			testFunc: func(t *testing.T) {
				total5StarProb, _ := banner.CalculateWarpProbability(banner.GenshinStandard, 89, 1, false)
		...
		},
		{
			...
			testFunc: func(t *testing.T) {
				total5StarProb, characterProb := banner.CalculateWarpProbability(banner.GenshinStandard, 0, 1, false)
...
func TestGenshinLimitedBanner(t *testing.T) {
	tests := []testCase{
		{
			name: "base rates are correct",
			testFunc: func(t *testing.T) {
				config := banner.GetConfig(banner.GenshinLimited)
				if config.BaseRate != 0.006 {
					t.Errorf("Expected base rate 0.6%%, got %.1f%%", config.BaseRate*100)
				}
				if config.FourStarRate != 0.051 {
		...
		},
		{
			...
			testFunc: func(t *testing.T) {
				total5StarProb, rateUpProb := banner.CalculateWarpProbability(banner.GenshinLimited, 0, 1, false)
		...
		},
		{
			...
			testFunc: func(t *testing.T) {
				total5StarProb, rateUpProb := banner.CalculateWarpProbability(banner.GenshinLimited, 0, 1, true)
...
func TestGenshinWeaponBanner(t *testing.T) {
	tests := []testCase{
		{
			name: "base rates are correct",
			testFunc: func(t *testing.T) {
				config := banner.GetConfig(banner.GenshinWeapon)
				if config.BaseRate != 0.007 {
					t.Errorf("Expected base rate 0.7%%, got %.1f%%", config.BaseRate*100)
				}
		...
		},
		{
			...
			testFunc: func(t *testing.T) {
				config := banner.GetConfig(banner.GenshinWeapon)
				if config.SoftPityStart != 62 {
					t.Errorf("Expected soft pity at 62, got %d", config.SoftPityStart)
				}
		...
		},
		{
			...
			testFunc: func(t *testing.T) {
				total5StarProb, _ := banner.CalculateWarpProbability(banner.GenshinWeapon, 79, 1, false)
				if !almostEqual(total5StarProb, 1.0) {
		...
		},
		{
			...
			testFunc: func(t *testing.T) {
				config := banner.GetConfig(banner.GenshinWeapon)
				beforePity, _ := banner.CalculateWarpProbability(banner.GenshinWeapon, config.SoftPityStart-1, 1, false)
				afterPity, _ := banner.CalculateWarpProbability(banner.GenshinWeapon, config.SoftPityStart, 1, false)
