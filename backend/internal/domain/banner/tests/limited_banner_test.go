...
func TestLimitedBanner(t *testing.T) {
	t.Run("base rates are correct", func(t *testing.T) {
		config := banner.GetBannerConfig(banner.Limited)
	...
	t.Run("maximum 180 pulls for guaranteed rate-up", func(t *testing.T) {
		...
		total5StarProb1, _ := banner.CalculateWarpProbability(banner.Limited, 89, 1, false)
		// Second 5★ at 180 pulls (worst case, after losing 50/50)
		total5StarProb2, rateUpProb2 := banner.CalculateWarpProbability(banner.Limited, 179, 1, true)
